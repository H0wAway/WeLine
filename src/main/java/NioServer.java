...
public class NioServer {
    ...
    private void processMsg(String message, SocketChannel client, SelectionKey key) throws IOException {
        ...
        String[] ms = message.split(seperator);
        if (ms.length == 1) {
            ...
            try (SocketChannel channel_to = onlineUsers.get(user_to)) {
                if (channel_to == null) {
                    client.write(charset.encode("用户'" + user_to + "'不存在，当前用户列表：" + onlineUsers.keySet().toString()));
                } else if (user_to.equals("ALL")) {
    ...
    private void chatQuery(String user_name) throws IOException {
        File file = new File("ChatRecord.json");
        File file2 = new File("ChatQuery.json");
        if (file2.exists())
            file2.delete();
        file2.createNewFile();
        try (Writer write = new OutputStreamWriter(new FileOutputStream(file2))) {
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file)))) {
                String line;
                Message msg;
                while ((line = reader.readLine()) != null) {
                    msg = JSON.parseObject(line, Message.class);

                }
            }
            // try-with-resource自动关闭,无需 write.close()
        }
    }
